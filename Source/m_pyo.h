...
INLINE PyThreadState * pyo_new_interpreter(float sr, int bufsize, int chnls) {
    ...
    */
    ...
    Py_InitModule("stdredirect",stdredirectMethods);
    PyRun_SimpleString("import sys; import stdredirect; sys.stdout = stdredirect; sys.stderr = stdredirect");
    ...
    /* On MacOS, change the current working directory to ~/Documents/PyoVST. */
    PyRun_SimpleString("from os.path import expanduser");
    PyRun_SimpleString("_home = expanduser('~')");
    PyRun_SimpleString("_cwd = _home+'/Documents/PyoVST'");
    PyRun_SimpleString("if not os.path.isdir(_cwd):os.mkdir(_cwd)");
    PyRun_SimpleString("os.chdir(_cwd)");
    ...
    /* Set P1 P2 P3 P4 SigTo objects */
    PyRun_SimpleString("P1 = SigTo(0., time=0.002, init=0.)");
    PyRun_SimpleString("P2 = SigTo(0., time=0.002, init=0.)");
    PyRun_SimpleString("P3 = SigTo(0., time=0.002, init=0.)");
    PyRun_SimpleString("P4 = SigTo(0., time=0.002, init=0.)");
...
INLINE unsigned long pyo_get_input_buffer_address(PyThreadState *interp) {
    ...
    address = (char*) PY_STRING_AS_STRING(obj);
